use std::{fs::{File, Metadata}, path::PathBuf, time::{UNIX_EPOCH, SystemTime}, io::{Seek, Write, Read, BufReader}, rc::Rc, collections::HashMap};

use crate::record::{Record, hash_sha1};
...
impl Manager {
    ...
    pub fn init(&mut self) {
        ...
        paths.for_each(|result| {
            ...
            let mut fd = File::options().read(true).write(true).open(file.path()).unwrap();
            fd.seek(std::io::SeekFrom::Start(0)).unwrap();
            let mut count = [0u8; 2];
            fd.read_exact(&mut count).unwrap();
            self.tables.insert(name.clone(), DiskTable {
                name,
                path: file.path(),
